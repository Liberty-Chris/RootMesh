...
unsigned long previousCalibrationTime = 0;
const unsigned long calibrationInterval = 300000; // 5 minutes in milliseconds
...
void loop() {
  ...
  // Calibrate the soil moisture sensors every 5 minutes
  if (currentMillis - previousCalibrationTime >= calibrationInterval) {
    calibrateSoilMoistureSensors();
    previousCalibrationTime = currentMillis;
  }
...
void calibrateSoilMoistureSensors() {
  Serial.println("Calibrating soil moisture sensors...");
  for (int i = 0; i < 5; i++) {
